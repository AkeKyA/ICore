...
class ChatSession extends PluginTask{
    ...
    public function sendMessage($message){
        if($this->state == 2){
            $this->player->sendMessage('§b[§cChatDefender§b] §cYou are currently blocked.');
            return false;
        }elseif($this->similarityCheck($message) || $this->rateCheck()){
            if($this->state == 1){
                $this->state = 2;
                $this->player->sendMessage('§b[§cChatDefender§b] §6You have been blocked for§b '.$this->blockLength * $this->getOwner()->getConfig()->get("baseblocklength") . ' §6second(s)');
                ...
                $this->state = 1;
                $this->player->sendMessage('§b[§cChatDefender§b] §cLooks like you are posting spammy stuff. Continuing to do so will result in a block.');
    ...
    public function onRun($tick){
        $this->state = 0;
        $this->player->sendMessage('§b[§cChatDefender§b] §aYour block has been lifted.');
    }
